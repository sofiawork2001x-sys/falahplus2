    ...
    <div className="space-y-6">
      ...
      {loading ? <p className="text-muted-foreground">جارٍ التحميل…</p> : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredLands.map((l) => (
            <Card key={l.id} id={l.id} title={l.title} desc={l.description} price={l.price} wilaya={wilayaName(wilayas, l.wilaya_code)} type="land" image={l.images[0]} sub={`${l.area_hectares} هكتار`} />
          ))}
          {filteredEquips.map((e) => (
            <Card key={e.id} id={e.id} title={e.name} desc={e.description} price={e.price} wilaya={wilayaName(wilayas, e.wilaya_code)} type="machine" image={e.images[0]} sub={e.category ?? "آلة"} />
          ))}
    ...
    <article className="overflow-hidden rounded-2xl border bg-card shadow-sm transition hover:shadow-md">
      ...
      <div className="space-y-2 p-5">
        ...
        <div className="flex items-center justify-between border-t pt-3">
          ...
          <Link to="/dashboard/payment" className="rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground hover:bg-primary/90">
            طلب كراء
          </Link>
        </div>
      </div>
